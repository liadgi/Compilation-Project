...
(define basic? 
	(lambda (x)
		(or (null? x) (integer? x) (string? x) (boolean? x) (char? x) (eq? (void) x))))
	...
	(lambda (constants-list counter)
			(letrec ((build 
				(lambda (lst table counter)
					(if (null? lst) table
						(if (or (eq? (car lst) (void)) (boolean? (car lst)) (null? (car lst)) (and (number? (car lst)) (or (eq? (car lst) 0) (eq? (car lst) 1) (eq? (car lst) -1))))
